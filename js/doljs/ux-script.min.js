...
LazyLoad = (function (doc) {
    ...
    function finish(type) {
        var callback,
            urls,
            p = pending[type];
        p && ((callback = p.callback), (urls = p.urls), urls.shift(), (pollCount = 0), urls.length || (callback && callback.call(p.context, p.obj), (pending[type] = null), queue[type].length && load(type)));
    }
    function getEnv() {
        var ua = navigator.userAgent;
        (env = { async: doc.createElement("script").async === !0 }), (env.webkit = /AppleWebKit\//.test(ua)) || (env.ie = /MSIE|Trident/.test(ua)) || (env.opera = /Opera/.test(ua)) || (env.gecko = /Gecko\//.test(ua)) || (env.unknown = !0);
    }
    function load(type, urls, callback, obj, context) {
        var i,
            ...
            nodes = [];
        if ((env || getEnv(), urls))
            if (((urls = "string" == typeof urls ? [urls] : urls.concat()), isCSS || env.async || env.gecko || env.opera)) queue[type].push({ urls: urls, callback: callback, obj: obj, context: context });
            else for (i = 0, len = urls.length; len > i; ++i) queue[type].push({ urls: [urls[i]], callback: i === len - 1 ? callback : null, obj: obj, context: context });
        if (!pending[type] && (p = pending[type] = queue[type].shift())) {
            for (head || (head = doc.head || doc.getElementsByTagName("head")[0]), pendingUrls = p.urls.concat(), i = 0, len = pendingUrls.length; len > i; ++i)
                (url = pendingUrls[i]),
                    isCSS ? (node = env.gecko ? createNode("style") : createNode("link", { href: url, rel: "stylesheet" })) : ((node = createNode("script", { src: url })), (node.async = !1)),
                    (node.className = "lazyload"),
                    node.setAttribute("charset", "utf-8"),
                    env.ie && !isCSS && "onreadystatechange" in node && !("draggable" in node)
                        ...
                        ? env.webkit
                            ...
                            : ((node.innerHTML = '@import "' + url + '";'), pollGecko(node))
                        : (node.onload = node.onerror = _finish),
                    nodes.push(node);
            for (i = 0, len = nodes.length; len > i; ++i) head.appendChild(nodes[i]);
        }
    }
    ...
    function pollWebKit() {
        ...
        if (css) {
            ...
            (pollCount += 1), css && (200 > pollCount ? setTimeout(pollWebKit, 50) : finish("css"));
        }
    }
    var env,
        head,
...
function inputDisabledStyle() {
    jQuery("input:disabled").addClass("disabled"), jQuery("input:enabled").removeClass("disabled");
}
if (!window.ux) {
    (window.ux = {}),
        (window.ux.core = {}),
        ...
        (ux.core.cssFolderSuffix = ""),
        (ux.core.jsFolderSuffix = ""),
        (ux.core.devMode = !1);
    var scriptEls = document.getElementsByTagName("script"),
        thisScriptEl = scriptEls[scriptEls.length - 1],
        scriptPath = thisScriptEl.src;
    (ux.core.resourcePath = scriptPath.substr(0, scriptPath.lastIndexOf("/v.") + 9)),
        (ux.core.version = ux.core.resourcePath.substr(scriptPath.lastIndexOf("/v.") + 1, scriptPath.lastIndexOf("/v.") + 9).replace("/", "")),
        (ux.features.alias = function (alias, loc) {
            window.ux[alias] ? console.log("Sorry you cant make a shortcute to:" + alias) : (window.ux[alias] = loc);
        }),
        ...
        (ux.features.startup = function (callback) {
            void 0 != window.beforeUX && "function" == typeof window.beforeUX && beforeUX(),
                ...
                ("chrome" == ux.core.browser || "safari" == ux.core.browser || "opera" == ux.core.browser) &&
                ...
                jQuery("#ux-side-nav ul li:first-child, #ux-footer ul li:first-child").addClass("first-child"),
                jQuery("#ux-side-nav ul li:last-child, #ux-footer ul li:last-child").addClass("last-child"),
                ...
                "msie" == ux.core.browser && jQuery("body").hasClass("ie7") && (jQuery(".ie7 tfoot tr:last-child").addClass("last-child"), jQuery(".ie7 .ux-wizard li:last-child").addClass("last-child")),
                "function" == typeof callback && callback(),
                document.getElementById("ux-side-nav") && ux.load("expandables"),
                document.getElementById("ux-side-nav-container") &&
                    document.getElementById("ux-side-nav-toggle") &&
                    jQuery("#ux-side-nav-toggle").on("click", function (e) {
                        e.preventDefault(),
                            e.stopPropagation(),
                            jQuery("#ux-domain-menu a.selected").length > 0 && jQuery("#ux-domain-menu a.selected").removeClass("selected").attr("aria-expanded", !1).next("div:first").hide().attr("aria-hidden", !0);
                        var link = jQuery(this),
                            container = jQuery("#ux-side-nav-container");
                        container.is(":visible")
                            ? (link.removeClass("active").attr("aria-expanded", !1), container.hide().attr("aria-hidden", !0), jQuery(document).off("click"), jQuery(document).off("keyup"))
                            : (link.addClass("active").attr("aria-expanded", !0),
                              container.show().attr("aria-hidden", !1),
                              jQuery(document).on("click", function () {
                                  link.removeClass("active").attr("aria-expanded", !1), container.hide().attr("aria-hidden", !0), jQuery(this).off("click");
                              }),
                              (document.onkeyup = function (evt) {
                                  (evt = evt || window.event), 27 == evt.keyCode && jQuery("#ux-side-nav-toggle").removeClass("active").attr("aria-expanded", !1).next("div:first").hide().attr("aria-hidden", !0).focus();
                              }));
                    }),
                jQuery(".ux-a-z-index").length &&
                    jQuery(".ux-a-z-index a").each(function () {
                        jQuery(this).on("click", function (event) {
                            event.preventDefault();
                            var selection = jQuery(this).text();
                            "#" == selection && (selection = "numbers"), switch_selected(selection);
                        });
                    }),
                jQuery("#gov-link3").length &&
                    jQuery("#gov-link3").on("click", function (e) {
                        e.preventDefault(), jQuery(this).hide().next("form").show().focus();
                    }),
                ux.domSaver(".ux-content a, footer a", "uxPageAnchors"),
                ux.load(["placeHolder", "tooltips"]),
                jQuery("#app-nav").length && ux.load("appMenu"),
                jQuery(".processing").length && ux.load("modalLoader");
        }),
        (ux.features.status = function () {
            var status =
                ...
                "\nBrowser detected: " +
                ux.core.browser +
                "\nFile Extention Prefix: " +
                ux.core.extPrefix +
                "\nFolder Suffix: " +
                ux.core.folderSuffix +
                "\nAsset Resource Path: " +
                ux.core.resourcePath +
                "\n\nLazyLoadable Library State:\n\n";
            for (func in ux.libraries) status += func + ": " + ux.libraries[func].isLoaded + "\n";
            status += "\n\nFunction State:\n\n";
            for (func in ux.functions) status += func + ": " + ux.functions[func].isLoaded + "\n";
            return console.log(status), !0;
        }),
        (ux.features.load = function (request, callback) {
            function testRequest(request, callback) {
                if (request in ux.libraries)
                    if (ux.libraries[request].isLoaded) ux.libraries[request].hasOwnProperty("deprecated") && console.log(ux.libraries[request].deprecated);
                    else {
                        if (ux.libraries[request].hasOwnProperty("dependencies")) {
                            typeof ux.libraries[request].dependencies;
                            ux.features.load(ux.libraries[request].dependencies);
                        }
                        loader(!0, ux.libraries[request], callback);
                    }
                else
                    request in ux.functions
            ...
            function loader(trusted, request, callback) {
                if (trusted) {
                    if (request.hasOwnProperty("styles")) {
                        ...
                        if ("string" == typeof styles) styleChecker(styles) && LazyLoad.css(assetPath(styles, "css"));
                        else for (var i = 0, r = styles.length; r > i; i++) styleChecker(styles[1]) && LazyLoad.css(assetPath(styles[i], "css"));
                    }
                    if (request.hasOwnProperty("script")) {
                        var afterLazyLoad = function () {
                            request.hasOwnProperty("defaultFunction") && request.defaultFunction(), "function" == typeof callback && callback(), (request.isLoaded = !0);
                        };
                        request.hasOwnProperty("specialPath") ? LazyLoad.js(assetPath(request.script, "js", request.specialPath), afterLazyLoad) : LazyLoad.js(assetPath(request.script, "js"), afterLazyLoad);
                    }
                }
            }
            function executer(request, callback) {
                ux.functions[request].isLoaded || (ux.functions[request].defaultFunction(), (ux.functions[request].isLoaded = !0));
            }
            function assetPath(request, type, specialPath) {
                var version = "",
                    folder = "";
                if (ux.core.devMode) var version = Math.floor(1001 * Math.random());
                return (
                    (folder += "js" == type ? type + ux.core.jsFolderSuffix : type + ux.core.cssFolderSuffix),
                    specialPath
                        ? ux.core.resourcePath + "/" + folder + "/" + specialPath + "/" + request + ux.core.extPrefix + "." + type + "?version=" + version
                        : ux.core.resourcePath + "/" + folder + "/" + request + ux.core.extPrefix + "." + type + "?version=" + version
                );
            }
            function styleChecker(file) {
                return jQuery("link[href*=" + file + "]").length >= 1 ? !1 : !0;
            }
            if ("string" == typeof request) testRequest(request, callback);
            else if ("object" == typeof request)
                if (request instanceof Array) for (var i = 0, r = request.length; r > i; i++) "string" == typeof request[i] ? testRequest(request[i]) : "object" == typeof request[i] && ux.features.load(request[i]);
                else if ("load" in request) {
                    var cbFunction = "callback" in request ? request.callback : void 0;
        ...
        (ux.features.override = function (request, obj, act) {
            function overrideAction(origObj, newObj, act) {
                if (act) {
                    for (member in origObj) delete member;
                    origObj = jQuery.extend(origObj, newObj);
                } else origObj = jQuery.extend(origObj, newObj);
            }
            request in ux.libraries && "defaultSettings" in ux.libraries[request]
                ? overrideAction(ux.libraries[request].defaultSettings, obj, act)
                : request in ux.functions && "defaultSettings" in ux.functions[request] && overrideAction(ux.functions[request].defaultSettings, obj, act);
        }),
        (ux.libraries = {
            dataTables: {
                script: "lazy-jquery.dataTables",
                styles: ["lazy-jquery-dataTables"],
                dependencies: "jqueryUI",
                defaultElement: ".jQDataTables",
                defaultSettings: { bPaginate: !0, sPaginationType: "full_numbers", bSort: !0, aaSorting: [], bInfo: !0, bAutoWidth: !0, bLengthChange: !1, bFilter: !0, bRetrieve: !0, bJQueryUI: !0 },
                defaultFunction: function () {
                    function jQDTExpand_FormatDetails(targetDiv) {
                        var expandData = targetDiv.html(),
                            sOut = '<div class="ux-jQDT-row-expanded-content">' + expandData + "</div>";
                        return sOut;
                    }
                    jQuery(".ux-jQDT-row-expandable").each(function () {
                        var trigger = jQuery(this).find("a.ux-jQDT-row-expand-trigger");
                        trigger.prepend('<img src="' + ux.core.resourcePath + '/images/icons/add.png" class="ux-jQDT-row-expand-img">'), jQuery(this).find(".ux-jQDT-row-expand-content").hide();
                    });
                    var jQDTExpand_icon_closed = ux.core.resourcePath + "/images/icons/add.png",
                        jQDTExpand_icon_opened = ux.core.resourcePath + "/images/icons/delete.png";
                    jQuery("a.ux-jQDT-row-expand-trigger").on("click", function () {
                        var thisTr = jQuery(this).closest("tr"),
                            nTr = thisTr[0],
                            targetDiv = jQuery(this).next(".ux-jQDT-row-expand-content"),
                            oTable = jQuery(this).closest("table.data-table").dataTable({ bRetrieve: !0 }),
                            trig_img = jQuery(this).find("img.ux-jQDT-row-expand-img")[0];
                        return (
                            trig_img.src.match(jQDTExpand_icon_opened)
                                ? ((trig_img.src = jQDTExpand_icon_closed), oTable.fnClose(nTr))
                                : (jQuery(thisTr)
                                      .find("a.ux-jQDT-row-expand-trigger")
                                      .each(function () {
            ...
            jqueryUI: { script: "lazy-jquery-ui-core", styles: ["lazy-jquery-ui"], isLoaded: !1 },
            jqueryTabs: {
            ...
            jqueryDatePicker: {
                ...
                defaultSettings: { showAnim: "", showOn: "both", buttonText: "Datepicker", buttonImage: ux.core.resourcePath + "/images/icons/calendar_disabled.png", buttonImageOnly: !0, changeMonth: !0, changeYear: !0 },
                defaultFunction: function () {
            ...
            jqueryAccordion: { script: "lazy-jquery-ui-accordion", dependencies: "jqueryUI", isLoaded: !1 },
            jqueryAutoComplete: { script: "lazy-jquery-ui-autocomplete", dependencies: "jqueryUI", isLoaded: !1 },
            jqueryButton: { script: "lazy-jquery-ui-button", dependencies: "jqueryUI", isLoaded: !1 },
            jqueryDialog: { script: "lazy-jquery-ui-dialog", dependencies: "jqueryButton", isLoaded: !1 },
            jquerySlider: { script: "lazy-jquery-ui-slider", dependencies: "jqueryUI", isLoaded: !1 },
            jqueryProgressBar: { script: "lazy-jquery-ui-progressbar", dependencies: "jqueryUI", isLoaded: !1 },
            jqueryDynaTree: { script: "lazy-jquery.dynatree", styles: "lazy-jquery-dynatree", dependencies: "jqueryUI", isLoaded: !1 },
            formEnhancements: { deprecated: "This functionality is no longer avaliable please lookup see the documentation about the split of show hide password and character counter functionality", isLoaded: !0 },
            characterCounter: { script: "lazy-jquery.open-ux.character-counter", styles: ["lazy-jquery-open-ux-character-counter"], isLoaded: !1 },
            showHidePassword: {
                script: "lazy-jquery.open-ux.show-hide-password",
                styles: "lazy-jquery-open-ux-show-hide-password",
                defaultElement: ".ux-password-show-hide",
                defaultSettings: { moreClasses: "button_mini no-icon" },
                defaultFunction: function () {
                    jQuery(this.defaultElement).showHidePass(this.defaultSettings);
                },
                isLoaded: !1,
            },
            keyboardShortcuts: { script: "lazy-jquery.open-ux.keyboard-shortcuts", styles: "lazy-jquery-open-ux-keyboard-shortcuts", dependencies: "jqueryDialog", isLoaded: !1 },
            notifications: { script: "lazy-jquery.open-ux.notifications", styles: "lazy-jquery-open-ux-notifications", isLoaded: !1 },
            navOutWarning: {
                ...
                defaultFunction: function (selector) {
                    selector || (ux.domSaver("a", "uxAllAnchors"), (selector = ux.dom.uxAllAnchors)), jQuery(selector).navOutWarning(this.defaultSettings);
                },
                isLoaded: !1,
            },
            fullCalendar: { script: "lazy-jquery.fullcalendar", styles: ["lazy-fullcalendar"], dependencies: "jqueryUI", isLoaded: !1 },
            video: { script: "lazy-video", styles: "lazy-video-js", isLoaded: !1 },
            sugar: { script: "lazy-sugar", isLoaded: !1 },
            swfobject: { script: "lazy-swfobject", isLoaded: !1 },
            popover: {
            ...
            textExpander: { script: "lazy-jquery.expander", styles: "lazy-jquery-expander", isLoaded: !1 },
            joyRide: { script: "lazy-jquery.joyride", styles: "lazy-jquery-joyride", isLoaded: !1 },
            validation: { script: "lazy-jquery.validationEngine", styles: ["lazy-jquery-validationEngine"], dependencies: "validationRegEx", isLoaded: !1 },
            validationRegEx: {
            ...
            appMenu: {
                script: "lazy-jquery.open-ux.app-menus",
                defaultElement: "#app-nav a",
                defaultFunction: function () {
                    jQuery(this.defaultElement).appMenu();
                },
                isLoaded: !1,
            },
            modalLoader: {
            ...
            richTextEditor: { script: "ckeditor", styles: "lazy-ckeditor-override", specialPath: "ckeditor", isLoaded: !1 },
            cookie: { script: "lazy-jquery.cookie", isLoaded: !1 },
            json2: { script: "lazy-json2", isLoaded: !1 },
            pageFeedback: { script: "lazy-jquery-page-feedback", styles: "lazy-jquery-page-feedback", dependencies: ["cookie", "json2"], isLoaded: !1 },
            tooltips: {
                script: "lazy-jquery.open-ux.tooltips",
                styles: "lazy-jquery-open-ux-tooltips",
                defaultElement: "abbr, acronym, dfn, .ux-contexthelp, .tooltip",
                defaultFunction: function () {
                    jQuery(this.defaultElement).tooltip();
                },
                isLoaded: !1,
            },
            timePicker: {
                script: "lazy-jquery-ui-addon-time-picker",
                styles: "lazy-jquery-ui-addon-time-picker",
                defaultElement: ".jQDateTimePicker",
                defaultSettings: {
                    showAnim: "",
                    showOn: "both",
                ...
                defaultFunction: function () {
                    jQuery(this.defaultElement).datetimepicker(this.defaultSettings);
                },
                dependencies: ["jqueryUI", "jqueryDatePicker"],
                isLoaded: !1,
            },
        }),
        (ux.functions = {
                ...
                defaultFunction: function () {
                    ux.features.domSaver("a.ux-expandable", "pageExpandables"),
                        ux.dom.pageExpandables.each(function () {
                            var obj = jQuery(this);
                            obj.parent().is("li") ? obj.parent().addClass("expandable-list") : obj.parent().hasClass("ux-closed") || "DIV" != obj.parent()[0].nodeName || obj.addClass("expanded");
                        }),
                        $("body").on("click", "a.ux-expandable", function (e) {
                            function checkSibs(obj) {
                                openClose(obj.parent("li").siblings("li").children(".ux-expandable.expanded")), openClose(obj);
                            }
                            function openClose(obj) {
                                var container = obj.next(),
                                    secondClass = "expanded";
                                obj.hasClass(secondClass)
                                    ? container.slideUp(function () {
                                          obj.removeClass(secondClass).attr("aria-expanded", !1), container.attr("aria-hidden", !0);
                                      })
                                    : (container.slideDown().attr("aria-hidden", !1), obj.addClass(secondClass).attr("aria-expanded", !0));
                            }
                            e.preventDefault(), e.stopPropagation();
                            var obj = jQuery(this),
                                onlyOne = !1;
                            obj.parents().is(".ux-only-one") && (onlyOne = !0), onlyOne ? checkSibs(obj) : openClose(obj);
                        });
                },
                isLoaded: !1,
            },
            placeHolder: {
                defaultFunction: function () {
                    jQuery('[class^="ux-attribute"]').each(function () {
                        (shim = jQuery(this).attr("class").replace(/ /g, ":").replace(/_/g, " ").split(":")), jQuery(this).attr(shim[1], shim[2]);
                    }),
                        Modernizr.input.placeholder ||
                            (jQuery("[placeholder]")
                                .focus(function () {
                                    var input = jQuery(this);
                                    input.val() === input.attr("placeholder") && (input.val(""), input.removeClass("ux-placeholder"), document.all && input.get(0).createTextRange().select());
                                })
                                .blur(function () {
                                    var input = jQuery(this);
                                    "" === input.val()
                                        ? (input.addClass("ux-placeholder"), input.val(input.attr("placeholder")))
                                ...
                                .change(function () {
                                    var input = jQuery(this);
                                    input.hasClass("ux-placeholder") && input.removeClass("ux-placeholder"), input.focus();
                                })
                                .blur(),
                            jQuery("[placeholder]")
                                .parents("form")
            ...
            translate: {
                ...
                defaultFunction: function () {
                    jQuery(".ux-translate-trigger").on("click", function () {
                        $(".app-translate").length && jQuery(this).uxNotice({ title: "Select Language", message: jQuery("#ux-translate-container").html(), type: "Required", classes: "app-translate" });
                    });
                },
                isLoaded: !1,
            },
            dismissibleMessages: {
                defaultFunction: function () {
                    jQuery(".ux-msg.ux-closable").each(function () {
                        var msg = jQuery(this),
                            closeElm = jQuery("<a>")
                                .addClass("ux-msg-close")
                                .html("&times;")
                                .on("click", function () {
                                    return msg.slideUp(), !1;
                                });
                        msg.prepend(closeElm);
                    });
                },
                isLoaded: !1,
            },
                        ...
                        jQuery("tr.expandable .icon .contracted").live("click", function () {
                            return (
                                jQuery(this)
                                    ...
                                    .addClass("expanded")
                                    .children()
                                    .attr("src", ux.core.resourcePath + "/images/background-navigation-expandable-down.gif")
                                    .attr("alt", "hide rows"),
                        ...
                        jQuery("tr.expandable .icon .expanded").live("click", function () {
                            return (
                                jQuery(this)
                                    ...
                                    .addClass("contracted")
                                    .children()
                                    .attr("src", ux.core.resourcePath + "/images/background-navigation-expandable-right.gif")
                                    .attr("alt", "show rows"),
